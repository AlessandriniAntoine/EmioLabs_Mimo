...
class ClosedLoopController(BaseController):
    ...
    def execute_control_at_camera_frame(self):
        ...
        if self.useObserver:
            ...
            # === Step 2: Update the observer state ===
            # TODO: Update the observer state based on the measurements and the command
            self.observerOutput = None # <<< to be completed
            self.observerState = None # <<< to be completed
        ...
        if self.guiNode.controlMode.value == ControlMode["State Feedback"]:
            # === Step 0: Compute the reduced state for control ===
            # TODO: Compute the reduced state for control based on the full state
            if not self.useObserver:
                fullState = np.vstack([self.legsPos, self.legsVel])
                state4Control = None # <<< to be completed
            # === Step 1: Compute the command ==
            # TODO: Compute the desired motor positions using the state feedback law
            desiredMotorPos = None # <<< to be completed

            desiredMotorPos = desiredMotorPos.flatten()
